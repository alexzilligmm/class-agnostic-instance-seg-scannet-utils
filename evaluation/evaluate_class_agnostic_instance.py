# Code adapted from SAI3D [CVPR2024] https://github.com/yd-yin/SAI3D
...
import evaluation.util_3d as util_3d
...
# exclude wall and floor, thus remaining 18 classes
...
def evaluate_matches(matches, args):
    # matches[abs_path] saves gt2pred and pred2gt
    overlaps = args.overlaps
    min_region_sizes = [ args.min_region_sizes[0] ]
    dist_threshes = [ args.distance_threshes[0] ]
    dist_confs = [ args.distance_confs[0] ]
    
    # results: class x overlap
    ap = np.zeros( (len(dist_threshes) , len(CLASS_LABELS) , len(overlaps)) , np.float16 )
...
def assign_instances_for_scan(pred_file, gt_file, pred_path, gt_path, args):
    ...
    for pred_mask_file in pred_info:
        ...
        if num < args.min_region_sizes[0]:
...
def evaluate(pred_files, gt_files, pred_path, output_file, args):
    print('evaluating', len(pred_files), 'scans...')
    matches = {}
    for i in tqdm.tqdm(range(len(pred_files)),desc='finding matches'):
        matches_key = os.path.abspath(gt_files[i])
        # assign gt to predictions
        gt2pred, pred2gt = assign_instances_for_scan(pred_files[i], gt_files[i], pred_path, args.gt_path, args)
        matches[matches_key] = {}
        ...
        sys.stdout.flush()
    print('')
    ap_scores = evaluate_matches(matches, args)
    avgs = compute_averages(ap_scores, args)
...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        description='Evaluate ScanNet predictions against ground truth.')
    
    parser.add_argument('--gt_path', type=str, required=True,
                        help='Path to ground truth .txt files')
    parser.add_argument('--pred_path', type=str, required=True,
                        help='Path to predicted .txt files')
    ...
    args.min_region_sizes     = np.array( [ 100 ] )
    # distance thresholds [m]
    args.distance_threshes    = np.array( [  float('inf') ] )
    # distance confidences
    args.distance_confs       = np.array( [ -float('inf') ] )
    return args
...
def main():
    ...
    pred_files = sorted([f for f in os.listdir(args.pred_path) if f.endswith('.txt')])
    if len(pred_files) == 0:
        util.print_error('No prediction files found.', user_fault=True)
        return

    gt_files = []
    valid_pred_files = []

    for pred_file in tqdm.tqdm(pred_files, desc="Matching prediction files"):
        gt_file = os.path.join(args.gt_path, pred_file)
        pred_file_full = os.path.join(args.pred_path, pred_file)

        if not os.path.isfile(gt_file):
            util.print_error(f' No ground truth file found for {pred_file}', user_fault=True)
            continue

        gt_files.append(gt_file)
        valid_pred_files.append(pred_file_full)

    if len(valid_pred_files) == 0:
        util.print_error('No valid (matched) files found for evaluation.', user_fault=True)
        return

    # --- Evaluate ---
    print(f"\nEvaluating {len(valid_pred_files)} file pairs...")
    evaluate(valid_pred_files, gt_files, args.pred_path, args.output_file, args)
