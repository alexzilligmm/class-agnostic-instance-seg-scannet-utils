...
def export_ids(filename, ids):
    if not os.path.exists(filename):
        ...
        print('making dir:', os.path.dirname(filename))
...
def read_mesh_vertices(filename):
    assert os.path.isfile(filename), f"{filename} not exist!"
...
def get_args():
    ...
    parser.add_argument('--combined', type=str, help='Where to save the pcd results')
...
if __name__ == '__main__':
    ...
    print("Arguments:")
    print(args)
    for scan in tqdm.tqdm(os.listdir(args.data_path)):
        if not os.path.isdir(os.path.join(args.data_path, scan)):
            continue
        scan_path = os.path.join(args.data_path, scan)
        save_scan_ids(scan_path, args.combined, args.save_path)
